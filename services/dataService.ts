...
export const dataService = {
  ...
  async returnSale(saleId: string) { return withRetry(async () => { const userId = await this.getUserId(); const { error } = await supabase.rpc('return_sale', { sale_uuid: saleId, user_uuid: userId }); if (error) throw error; }); },
  async returnPurchase(purchaseId: string) { return withRetry(async () => { const userId = await this.getUserId(); const { error } = await supabase.rpc('return_purchase', { purchase_uuid: purchaseId, user_uuid: userId }); if (error) throw error; }); },
  ...
  async updateProfile(userId: string, updates: any) { return withRetry(async () => { const { error } = await supabase.from('users').update(updates).eq('id', userId); if (error) throw error; }); },
  async updateSettings(userId: string, updates: any) { return withRetry(async () => { const { error } = await supabase.from('user_settings').update(updates).eq('user_id', userId); if (error) throw error; }); },
